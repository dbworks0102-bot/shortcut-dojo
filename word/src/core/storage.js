...
// ── ハイスコア ──
export function getHi()  { return parseInt(localStorage.getItem('word-dojo-hi') || '0') }
export function setHi(v) { localStorage.setItem('word-dojo-hi', v) }

// ── プレイヤー名 ──
export function getPlayerName() {
  return localStorage.getItem('word-dojo-name') || ''
}
export function savePlayerName(name) {
  const n = name.trim() || 'ゲスト'
  localStorage.setItem('word-dojo-name', n)
  return n
}
...
export function getRankKey(diff) { return 'word-dojo-ranks-' + diff }
...
// ── プレイヤー統計（マイページ） ──
export function getStatsKey(name) { return 'word-dojo-mystats-' + (name || 'ゲスト') }
