...
export async function generateShareImage({ grade, score, correct, wrong, acc, maxCombo, diff, playerName }) {
  ...
  bar.addColorStop(0,    '#2b579a')
  bar.addColorStop(0.33, '#5b9bd5')
  bar.addColorStop(0.66, '#1c4587')
  bar.addColorStop(1,    '#2b579a')
  ctx.fillStyle = bar
  ...
  ctx.fillText('🥋  Word道場', W / 2, 82)

  ctx.font         = '26px "Noto Sans JP", "Segoe UI", sans-serif'
  ctx.fillStyle    = '#8892b0'
  ctx.fillText('Word ショートカットキー 練習ゲーム', W / 2, 122)
  ...
  ctx.textAlign    = 'right'
  ctx.fillText('shortcut-dojo.com/word/', W - 36, H - 22)
...
export async function shareScore(params) {
  ...
  const shareUrl = 'https://shortcut-dojo.com/word/'

  // ── Web Share API (スマホ・画像付き) ──
  if (navigator.share) {
    try {
      const blob = await generateShareImage(params)
      const file = new File([blob], 'word-dojo-score.png', { type: 'image/png' })

      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ title: 'Word道場 スコア', text: shareText, files: [file], url: shareUrl })
    ...
    try {
      await navigator.share({ title: 'Word道場 スコア', text: shareText, url: shareUrl })
  ...
  try {
    ...
    a.download = 'word-dojo-score.png'
    a.click()
