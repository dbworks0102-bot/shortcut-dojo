...
// ── ハイスコア ──
export function getHi()  { return parseInt(localStorage.getItem('excel-dojo-hi') || '0') }
export function setHi(v) { localStorage.setItem('excel-dojo-hi', v) }

// ── プレイヤー名 ──
export function getPlayerName() {
  return localStorage.getItem('excel-dojo-name') || ''
}
export function savePlayerName(name) {
  const n = name.trim() || 'ゲスト'
  localStorage.setItem('excel-dojo-name', n)
  return n
}
...
export function getRankKey(diff) { return 'excel-dojo-ranks-' + diff }
...
// ── プレイヤー統計（マイページ） ──
export function getStatsKey(name) { return 'excel-dojo-mystats-' + (name || 'ゲスト') }
