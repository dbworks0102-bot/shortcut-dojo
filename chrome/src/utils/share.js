...
export async function generateShareImage({ grade, score, correct, wrong, acc, maxCombo, diff, playerName }) {
  ...
  ctx.fillText('🥋  Excel道場', W / 2, 82)

  ctx.font         = '26px "Noto Sans JP", "Segoe UI", sans-serif'
  ctx.fillStyle    = '#8892b0'
  ctx.fillText('Excel ショートカットキー 練習ゲーム', W / 2, 122)
  ...
  if (navigator.share) {
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ title: 'Excel道場 スコア', text: shareText, files: [file], url: shareUrl })
    ...
    try {
      await navigator.share({ title: 'Excel道場 スコア', text: shareText, url: shareUrl })
